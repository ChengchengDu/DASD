    ...
    def forward(
        ...
        if labels is not None:
            if label_smoothing == 0:
                ...
                print("summary loss", summary_loss)

        kl_loss = None
        if teacher_logits is not None:
            print("teacher_logtis")
            print(teacher_logits.shape)
            print("student logits")
            print(lm_logits.shape)
            assert teacher_logits.shape == lm_logits.shape
            soft_criterion = nn.KLDivLoss(reduction='batchmean')
            kl_loss = soft_criterion(F.log_softmax(lm_logits), F.log_softmax(teacher_logits))

        # 计算instance labels的损失

        ins_summary_loss = None
        if ins_labels is not None:
            if label_smoothing == 0:
                loss_fct = nn.CrossEntropyLoss(ignore_index=self.config.pad_token_id)
                assert lm_logits.shape[-1] == self.model.config.vocab_size
                ins_summary_loss = loss_fct(lm_logits.view(-1, lm_logits.shape[-1]), ins_labels.view(-1))

            else:
                lprobs = F.log_softmax(lm_logits, dim=-1)
                ins_summary_loss, ins_summary_nll_loss = label_smoothed_nll_loss(
                    lprobs, ins_labels, label_smoothing, ignore_index=self.config.pad_token_id
                )
            print("ins_summary_loss", ins_summary_loss)
        ...
        if kl_loss is not None:
            total_loss += kl_loss
        if ins_labels is not None:
            assert ins_summary_loss is not None
            total_loss = (1 - alpha) * summary_loss + alpha * ins_summary_loss
        if not return_dict:
