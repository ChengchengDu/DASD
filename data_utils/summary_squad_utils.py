...
def write_source_to_path(args):
    ...
    query_path = os.path.join(args.data_dir, args.data_type + ".query")
    ...
    query_save_path = os.path.join(args.save_path, args.data_type + '_query')
    ...
    with open(source_path, 'r', encoding="utf-8") as source, \
            open(target_path, 'r', encoding="utf-8") as target, \
                open(query_path, 'r', encoding="utf-8") as query:
        source_lines = source.readlines()
        target_lines = target.readlines()
        query_lines = query.readlines()
        for index, (source_line, target_line, query_line) in tqdm(enumerate(zip(source_lines, target_lines, query_lines))):
            if not source_line.strip() or not target_line.strip() or not query_line.strip():
                continue
            ...
            querys_new.append(query_line.strip() + "\n")
            ...
            with open(os.path.join(query_save_path, str(index_) + ".txt"), 'w', encoding="utf-8") as qf:
                qf.write(query_line.strip())
    ...
    with open(os.path.join(save_path, args.data_type + ".query"), 'w', encoding="utf-8") as w:
        w.writelines(querys_new)
    assert len(sources_new) == len(targets_new)
    assert len(sources_new) == len(querys_new)
...
def nema2squad(args):
    # 1. 抽取共同的实体信息  作为答案
    # 2. 构建squad数据集
    source_path = args.source_nema_path
    target_ie_path = args.target_nema_ie_path
    query_path = args.query_nema_path
    target_path = args.target_nema_path
    source_ie_path = args.source_nema_ie_path

    source_files = os.listdir(source_path)
    target_ie_files = os.listdir(target_ie_path)
    query_files = os.listdir(query_path)
    target_files = os.listdir(target_path)
    source_ie_files = os.listdir(source_ie_path)

    corpus_type = args.corpus_nema_type

    source_files.sort(key=lambda x: int(x.split('.')[0]))
    target_ie_files.sort(key=lambda x: int(x.split('.')[0]))
    query_files.sort(key=lambda x: int(x.split('.')[0]))
    target_files.sort(key=lambda x: int(x.split('.')[0]))
    source_ie_files.sort(key=lambda x: int(x.split('.')[0]))

    source_paths = [os.path.join(source_path, f) for f in source_files]
    target_ie_paths = [os.path.join(target_ie_path, f) for f in target_ie_files]
    query_paths = [os.path.join(query_path, f) for f in query_files]
    target_paths = [os.path.join(target_path, f) for f in target_files]
    source_ie_paths = [os.path.join(source_ie_path, f) for f in source_ie_files]

    assert len(source_paths) == len(target_ie_paths)
    assert len(query_paths) == len(source_paths)
    assert len(target_paths) == len(source_paths)

    dct = {'data': [], 'version': "nema" + "-" + corpus_type + "-squad1.1"}  # 最终的json字典

    for (source_file, target_ie_file, query_file, target_file, source_ie_file) in zip(
            source_paths, target_ie_paths, query_paths, target_paths, source_ie_paths):
        with open(source_file, 'r', encoding="utf-8") as sf, open(target_ie_file, 'r', encoding="utf-8") as tif, \
                open(query_file, 'r', encoding="utf-8") as qf, open(target_file, 'r', encoding="utf-8") as taf, \
                open(source_ie_file, 'r', encoding="utf-8") as sif:
            # 获取ie信息
            target_ie_data = json.load(tif)
            source_ie_data = json.load(sif)
            # 获取原文档
            source_line = sf.readline().strip()
            query_line = qf.readline().strip()
            target_line = taf.readline().strip()
            answers = get_nema_answer(
                source_line=source_line, target_ie_data=target_ie_data, source_ie_data=source_ie_data)

            if not answers:
                continue

            text = tuple(answers.items())[0][0]
            start_pos = tuple(answers.items())[0][1]
            ...
            # 对summary进行分句子 然后得到每一个自然句子的开始和结束的下标
            # 用于后面互信息的计算时选择最相似的进行计算
            paragraph['summary_start_poses'] = [0]
            paragraph['summary_end_poses'] = [len(target_line) - 1]
            qas = []
            cur_qas = {}
            cur_qas['question'] = query_line
            cur_qas['id'] = hashhex(query_file)
            answers = []
            answer = {}
            answer['answer_start'] = start_pos
            answer['text'] = text
            answers.append(answer)
            cur_qas['answers'] = answers
            qas.append(cur_qas)
    ...
    if not os.path.exists(args.save_nema_json_path):
        os.makedirs(args.save_nema_json_path)
    write_path = os.path.join(args.save_nema_json_path, dct["version"] + ".json")

    print("data length", len(dct['data']))
...
def get_nema_answer(source_line, target_ie_data, source_ie_data):
    target_tokens, target_ner, target_ie = get_ie(target_ie_data)
    source_tokens, source_ner, source_ie = get_ie(source_ie_data)

    answers = {}
    if target_ner:
        for ner in target_ner.keys():
            if ner in source_line:
                start = source_line.find(ner)
                answers[ner] = start

    elif not answers and target_tokens:
        for token in target_tokens:
            if token in source_line:
                start = source_line.find(token)
                answers[token] = start
    elif not answers and source_ner:
        for ner in source_ner.keys():
            start = source_line.find(ner)
            answers[ner] = start
            break

    elif not answers and source_tokens:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--source_nema_path', default="./", type=str
    )
    parser.add_argument(
        '--target_nema_ie_path', default='./', type=str
    )
    parser.add_argument(
        '--query_nema_path', default='./', type=str
    )
    parser.add_argument(
        '--target_nema_path', default='./', type=str
    )
    parser.add_argument(
        '--source_nema_ie_path', default='./', type=str
    )
    parser.add_argument(
        '--corpus_nema_type', default='train', type=str
    )
    parser.add_argument(
        '--save_nema_json_path', default='./', type=str
    )
