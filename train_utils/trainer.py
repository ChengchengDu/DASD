...
def trainer(
    ...
    for epoch in tqdm(train_iterator, desc="Epoch"):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            summary_labels = batch["labels"].long().to(args.device)
            if args.hard_distill:
                ins_summary_labels = batch["ins_labels"].long().to(args.device)
            else:
                ins_summary_labels = None
            ...
            try:
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        # rouge12_score = rouge_validation(
                        #     valid_dataloader, model, args
                        # )
                        rouge12_score = 0.0
                        if best_rouge < rouge12_score:
                            best_rouge = rouge12_score
                            logger.info('Saving best valid rouge12 model at global_steps{}'.format(global_step))
                            best_valid_rouge_dir = os.path.join(args.output_dir, "best_rouge")
                            if not os.path.exists(best_valid_rouge_dir):
                                os.makedirs(best_valid_rouge_dir)

                            model_to_save = model.module if hasattr(model, "module") else model
                            model_to_save.save_pretrained(best_valid_rouge_dir)
                            logger.info("Saving model checkpoint to %s", best_valid_rouge_dir)

                            torch.save(optimizer.state_dict(), os.path.join(best_valid_rouge_dir, "optimizer.pt"))
                            torch.save(scheduler.state_dict(), os.path.join(best_valid_rouge_dir, "scheduler.pt"))
                            logger.info("Saving optimizer and scheduler states to %s", best_valid_rouge_dir)
...
def rouge_validation(validation_dataloader, model, args):
    sys.path.append("/home/jazhan/code/qfs_pure/")
    from train_utils.run_eval import com_score
    ...
    with torch.no_grad():
        for step, batch in enumerate(epoch_valid_dataloader):
            model.eval()
            source_ids = batch["source_ids"].long().to(args.device)
            attention_mask = batch["source_mask"].long().to(args.device)
            ...
            for hypothesis in dec:
               outputs_lines.append(hypothesis + "\n")
        refer_path = os.path.join(args.data_dir, "val.target")
        with open(refer_path, 'r', encoding="utf-8") as f:
            reference_lines = f.readlines()
        
        score = com_score(outputs_lines, reference_lines)
        # print("evaluation rouge score is:")
        # print(score)
        rouge1, rouge2, rougel = score["rouge1"], score["rouge2"], score["rougeL"]
        eval_score = (rouge1 + rouge2) / 2     
        print("evaluate rouge1:{}, rouge2:{}".format(rouge1, rouge2))
...
def validation(validation_dataloader, model, args):
    ...
    valid_loss = 0.0
    valid_qa_loss = 0.0  # encoder损失
    valid_sum_loss = 0.0   # decoder损失
    valid_mim_loss = 0.0
    global_steps = 0.0

    total_correct_num = 0.0
    total_non_pad_num = 0.0
    ...
    with torch.no_grad():
        ...
        valid_loss = valid_loss / float(global_steps)
        valid_sum_loss = valid_sum_loss / float(global_steps)
        valid_qa_loss = valid_qa_loss / float(global_steps)
        valid_mim_loss = valid_mim_loss / float(global_steps)
        logger.info(
